...
function Education({ title, items }: EducationProps) {
  return (
    <Wrapper>
      <Title>{title}</Title>
      <Content>
            ...
            return (
              ...
              >
                <Card
                  ...
                  cover={<img style={{ padding: 1 }} alt={item.title} src={item.logo} />}
                >
                  <Meta
                    title={<b style={{ fontSize: '1.3em', lineHeight: 2 }}>{item.title}</b>}
                    description={item.descriptions.map((description, index) => {
                      return (
                        <p key={index} style={{ color: '#000000', opacity: 0.79 }}>
                          {description}
                        </p>
                      );
                    })}
